...
bool ShapePublisher::init(bool with_security)
{
    ...
    topic1 = participant1->create_topic("target_update", type_.get_type_name(), topic_qos);
    topic2 = participant2->create_topic("target_update", type_.get_type_name(), topic_qos);
    topic3 = participant3->create_topic("target_update", type_.get_type_name(), topic_qos);
            ...
            {   std::cout << "Inside loop Destination IP: " << destination_ip << std::endl;
                ...
                {   //std::cerr << "yesssss" << std::endl;
                   // sample.topics().emplace_back("command");
                    sample.data(last_command_payload);
                ...
                {    //std::cerr << "yesssss" << std::endl; 
                    ...
                    sample.data(last_swamp_gcs_payload);
                    if (destination_ip == "239.255.0.1")
                    {   std::cerr << "yesssss" << std::endl;
                        writer1->write(&sample);
                ...
                {   //std::cerr << "yesssss" << std::endl;
                    ...
                    sample.data(last_target_update_payload);
                    if (destination_ip == "239.255.0.1")
                    {   std::cerr << "yesssss" << std::endl;
                        writer1->write(&sample);

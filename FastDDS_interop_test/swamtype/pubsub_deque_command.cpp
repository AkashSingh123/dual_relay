...
std::deque<NetboxMessage1> message_queue;  // Deque to store received messages
    ...
    public:
        ...
        void on_data_available(DataReader *reader) override
        {
            ...
            if (return_code == ReturnCode_t::RETCODE_OK)
            {
                ...
                message_queue.push_back(sample);  // Push the received message to the back of the deque
                std::cout << "Message queued. Queue size: " << message_queue.size() << std::endl;
            }
            else
            {
...
bool CombinedPubSub::init()
{
   ...
   sub_participant1_ = DomainParticipantFactory::get_instance()->create_participant(1, sub_participant_qos);
    ...
    datareader_qos.latency_budget().duration = eprosima::fastrtps::Duration_t(0, 100000000); // 100 milliseconds

   // datareader_qos.reliability().reliable_reader_qos().nackResponseDelay = eprosima::fastrtps::Duration_t(0, 100000); // 0.1 ms response delay
   // datareader_qos.reliability().reliable_reader_qos().max_heartbeat_response_delay = eprosima::fastrtps::Duration_t(0, 100000); // Fast heartbeat response
 ...
 //   datareader_qos.deadline().period = eprosima::fastrtps::Duration_t(2, 0); // 2 seconds
   ...
   // datareader_qos.latency_budget().duration = eprosima::fastrtps::Duration_t(0, 50000000); // 50 milliseconds
   ...
   //datareader_qos.liveliness().lease_duration = eprosima::fastrtps::c_TimeInfinite; // Set an appropriate lease duration
    ...
    datareader_qos.resource_limits().max_samples = std::numeric_limits<int32_t>::max();
    datareader_qos.resource_limits().max_instances = std::numeric_limits<int32_t>::max();
    datareader_qos.resource_limits().max_samples_per_instance = std::numeric_limits<int32_t>::max();

    
    std::cout << "History QoS kind: " 
              << (datareader_qos.history().kind == KEEP_LAST_HISTORY_QOS ? "KEEP_LAST" : "KEEP_ALL") 
              << std::endl;
    std::cout << "History QoS depth: " << datareader_qos.history().depth << std::endl;
 ...
 //   topic_qos1.reliability().kind = RELIABLE_RELIABILITY_QOS;
   ...
   // topic_qos.history().kind = KEEP_ALL_HISTORY_QOS;  // Keep all history
    ...
    datawriter_qos.reliable_writer_qos().times.heartbeatPeriod.seconds = 1;
...
void CombinedPubSub::process_queue()
{
    while (!stop_signal)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(mtx);
            NetboxMessage1 sample = message_queue.front();
            message_queue.pop_front();  // Remove the message from the front of the deque
            sample.id(-1);
            listener1_.writer1_->write(&sample);
            std::cout << "Message sent, ID: " << sample.id() << " | Queue size: " << message_queue.size() << std::endl;
        }
...
void CombinedPubSub::run()
{
    ...
    std::thread writer_thread(&CombinedPubSub::process_queue, this);

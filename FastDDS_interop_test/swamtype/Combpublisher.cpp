...
void capture_rtps() {
    ...
    auto packetHandler = [](u_char* userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
        ...
        if (magic + 24 <= packet + pkthdr->len && magic[20] == 0x15) {
            ...
            // Initialize the NetboxMessage sample early
            ...
            // Extract topics and payloads
            size_t pos = 0;
            size_t last_topic_end = 0;

            bool found_topic = false;

            while (pos < payload_hex.size()) {
                size_t topic_pos = std::string::npos;
                std::string temp_topic;
                if ((topic_pos = payload_hex.find(hex_command, pos)) != std::string::npos) {
                    temp_topic = "command";
                    sample.topics().emplace_back("command");
                    found_topic = true;
                } else if ((topic_pos = payload_hex.find(hex_zones, pos)) != std::string::npos) {
                    temp_topic = "zones";
                    sample.topics().emplace_back("zones");
                    found_topic = true;
                } else if ((topic_pos = payload_hex.find(hex_swamp_gcs, pos)) != std::string::npos) {
                    temp_topic = "swamp_gcs";
                    sample.topics().emplace_back("swamp_gcs");
                    found_topic = true;
                } else if ((topic_pos = payload_hex.find(hex_target_update, pos)) != std::string::npos) {
                    temp_topic = "target_update";
                    sample.topics().emplace_back("target_update");
                    found_topic = true;
                 } else {
                    break; // No more known topics found
                }

                // Update last topic end position
                last_topic_end = topic_pos + temp_topic.size();

                // Move position to the end of the found topic
                pos = topic_pos + temp_topic.size();
            }
            ...
            if (payload_start_pos < payload_hex.size()) {
                ...
                std::string payload_segment = payload_hex.substr(payload_start_pos, payload_length);
...
std::vector<uint8_t> nice_load = {
    0x00, 0x17, 0xfb, 0xc9, 0x63, 0x0b, 0x07, 0x11, 0x01, 0x00, 0x00, 0x01, 0x02, 0x00, 0x00, 0x20, 0x41, 0x00,
    0x00, 0x20, 0x41, 0x00, 0x00, 0x40, 0x40, 0x00, 0x17, 0xfb, 0xca, 0x63, 0x0b, 0x07, 0x11, 0x01, 0x00,
    0x00, 0x01, 0x02, 0x00, 0x00, 0x20, 0x41, 0x00, 0x00, 0x20, 0x41, 0x00, 0x00, 0x40, 0x40
};
    
    
    
    sample.received_payload() = nice_load;

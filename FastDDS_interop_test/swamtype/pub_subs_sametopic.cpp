...
private:
    ...
    public:
        ...
        {
          std::cout << "start >> " << writer1_ << " " << writer2_ << std::endl; 
            ...
            std::cout << "Address of sample : " << &sample << " " << reader << std::endl;
            if (return_code == ReturnCode_t::RETCODE_OK && info.valid_data)
            {
                ...
                if (sample.id() == 0)
                {
                    ...
                    // Change the ID before sending it out
           ...
           std::cout << "end >> " << writer1_ << " " << writer2_ << std::endl; 
        }
...
bool CombinedPubSub::init()
{
    ...
    sub_participant_qos1.transport().send_socket_buffer_size = 1048576;  // Set send buffer to 1MB
    sub_participant_qos1.transport().listen_socket_buffer_size = 8194304;  // Set receive buffer to 4MB
  ...
  //  datawriter_qos1.reliability().max_blocking_time = eprosima::fastrtps::Duration_t(1, 0);  // 1 second max blocking time
   ...
   //  listener2_ = PubSubListener(writer1_);
     listener1_.writer2_ = writer2_;
     listener2_.writer1_ = writer1_;
    return true;
}
...
private:
    ...
    {
    public:
        PubSubListener(DataWriter* writer)
            : writer_(writer), matched(0), received_samples(0) {}
        ...
        void on_data_available(DataReader *reader) override
        {
            ...
            if (return_code == ReturnCode_t::RETCODE_OK && info.valid_data)
            {
                ...
                {
                    ++received_samples_;
                    std::cout << "Message received with ID 0. Total messages received: " << received_samples_ << std::endl;
...
bool CombinedPubSub::init()
{
    // First Subscriber and Publisher pair: Reads from Domain 7, writes to Domain 1

    // Subscriber (Domain 7)
    DomainParticipantQos sub_participant_qos1;
    sub_participant_qos1.name("subscriber_participant1");

    sub_participant1_ = DomainParticipantFactory::get_instance()->create_participant(7, sub_participant_qos1);
    if (!sub_participant1_)
        return false;
    ...
    eprosima::fastrtps::rtps::Locator_t unicast_locator1;
    unicast_locator1.kind = LOCATOR_KIND_UDPv4;
    eprosima::fastrtps::rtps::IPLocator::setIPv4(unicast_locator1, "192.168.50.29");
    ...
    sub_topic1_ = sub_participant1_->create_topic("7", type1_.get_type_name(), topic_qos1);
    if (!sub_topic1_)
        return false;

    listener1_ = PubSubListener(nullptr);  // The writer will be set up later

    reader1_ = subscriber1_->create_datareader(sub_topic1_, datareader_qos1, &listener1_);
    if (!reader1_)
        return false;
    ...
    listener1_.writer_ = writer1_;  // Set the writer for listener1

    // Second Subscriber and Publisher pair: Reads from Domain 1, writes to Domain 7

    // Subscriber (Domain 1)
    DomainParticipantQos sub_participant_qos2;
    sub_participant_qos2.name("subscriber_participant2");

    sub_participant2_ = DomainParticipantFactory::get_instance()->create_participant(1, sub_participant_qos2);
    if (!sub_participant2_)
        return false;
    ...
    eprosima::fastrtps::rtps::Locator_t unicast_locator2;
    unicast_locator2.kind = LOCATOR_KIND_UDPv4;
    eprosima::fastrtps::rtps::IPLocator::setIPv4(unicast_locator2, "192.168.50.29");
    ...
    sub_topic2_ = sub_participant2_->create_topic("7", type1_.get_type_name(), topic_qos1);
    if (!sub_topic2_)
        return false;

    listener2_ = PubSubListener(nullptr);  // The writer will be set up later

    reader2_ = subscriber2_->create_datareader(sub_topic2_, datareader_qos2, &listener2_);
    if (!reader2_)
        return false;
    ...
    listener2_.writer_ = writer2_;  // Set the writer for listener2

...
int main(int argc, char** argv) {
    ...
    if (publisher.init(false) && subscriber.init(false)) {
        std::thread publisher_thread(&ShapePublisher::run, &publisher);
        std::thread subscriber_thread(&ShapeSubscriber::run, &subscriber);

        publisher_thread.join();
        subscriber_thread.join();
    } else {
        std::cerr << "Failed to initialize publisher or subscriber." << std::endl;
    }

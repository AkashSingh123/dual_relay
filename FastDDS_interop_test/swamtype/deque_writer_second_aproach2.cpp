...
private:
    ...
    public:
        PubSubListener(DataReader* reader, DataWriter* writer, int msg_id)
            : reader_(reader), writer_(writer), msg_id_(msg_id),  matched(0), received_samples(0) {}
  ...
  //  datawriter_qos1.reliability().max_blocking_time = eprosima::fastrtps::Duration_t(1, 0);  // 1 second max blocking time
   ...
   //  listener2_ = PubSubListener(writer1_);
    ...
    reader2_->set_listener(&listeners_.back());
      ...
      if (!reader1_ || !reader2_ || !writer1_ || !writer2_) {
        std::cerr << "Failed to create DataReaders or DataWriters." << std::endl;
        return false;
     }
        ...
        void on_data_available(DataReader *reader) override
        {
            ...
            if (return_code == ReturnCode_t::RETCODE_OK && info.valid_data)
            {
                ...
                if (sample.id() == 0)
                {
                    ...
                    if (writer1_) {
                        std::lock_guard<std::mutex> lock(mtx);  // Lock the mutex for thread-safe access
                        sample.id(999);  // Assign ID for reader1
                        message_queue.push_back(sample);  // Add to deque
                    } else if (writer2_) {
                        std::lock_guard<std::mutex> lock(mtx);  // Lock the mutex for thread-safe access
                        sample.id(1000);  // Assign ID for reader2
                        message_queue.push_back(sample);  // Add to deque
                    }
       ...
       // std::lock_guard<std::mutex> lock(mtx);  // Lock the mutex for thread-safe access
        if (!message_queue.empty())
        {
          //  std::this_thread::sleep_for(std::chrono::microseconds(40));
            ...
            if (message.id() == 999 && writer1_)
            {
                writer1_->write(&message);  // Send using writer2
                std::cout << "Message sent by writer2 with ID: " << message.id() << std::endl;
            }
            else if (message.id() == 1000 && writer2_)
            {
                writer2_->write(&message);  // Send using writer1
                std::cout << "Message sent by writer1 with ID: " << message.id() << std::endl;
            }
        }
    }
}

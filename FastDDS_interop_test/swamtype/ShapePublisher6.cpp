...
//std::vector<Config> configs; // Define the static member
...
ShapePublisher::~ShapePublisher()
{
    for (auto& pub_data : participants_data)
    {
        ...
        {
            if (writer_data.writer && pub_data.publisher)
            {
                pub_data.publisher->delete_datawriter(writer_data.writer);
            }
        }
        if (pub_data.publisher && pub_data.participant)
        {
            pub_data.participant->delete_publisher(pub_data.publisher);
        }
        if (pub_data.participant)
        {
            DomainParticipantFactory::get_instance()->delete_participant(pub_data.participant);
        }
    }
}
...
void capture_rtps() {
    ...
    handle = pcap_open_live(device->name, BUFSIZ, 1, 10, errorBuffer);
    if (!handle) {
        ...
        if (writerEntityId == "00 00 0e 02 " || writerEntityId == "00 00 12 02 " ||
            ...
            for (const auto& config : configs) {
                ...
                if (destination_ipcheck == config.source_destination_ip) {
                    destination_ip = config.source_destination_ip;
...
void ShapePublisher::run()
{
    ...
    //    cv.wait(lock, []{ return new_data; });
        ...
        for (auto& pub_data : participants_data)
        {
            for (auto& writer_data : pub_data.writers_data)
            {
                if (writer_data.source_destination_ip == destination_ip)
                {
...
void ShapePublisher::run()
{
    ...
    while (!publisher_stop::stop)
    {  if (captured_topic == "target_update"){log_file << "came in0" << std::endl;}

       std::unique_lock<std::mutex> lock(mtx);
       cv.wait(lock, []{ return new_data; });
        if (captured_topic == "target_update"){log_file << "came in1" << std::endl;}
        ...
        {   
           ...
           if (send_data)
              if (captured_topic == "target_update"){log_file << "came in2" << std::endl;}
                  ...
                  //         std::cout << "Sending payload to: " << destination_ip << std::endl;
                   ...
                   //        std::cout << "Sending payload to: " << destination_ip << std::endl;
                       ...
                       else if (captured_topic == "target_update")
                       {
                           ...
                           sample.payload(last_target_update_payload);
                           log_file << "sending to target update" << std::endl;
                      ...
                      writer_data.writer->write(&sample);
                      found_writer = true; // Set the flag
                      break; // Break out of the inner loop

                  }

              }
              if (found_writer) break;
          }

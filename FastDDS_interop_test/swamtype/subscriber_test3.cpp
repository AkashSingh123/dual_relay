...
bool ShapeSubscriber::init(bool with_security)
{
  ...
  //  datareader_qos2.durability().kind = VOLATILE_DURABILITY_QOS;
   /* datareader_qos2.resource_limits().max_samples = 3000000000; // Set a high number to accumulate more data
    datareader_qos2.resource_limits().max_instances = 1110;  // Adjust as needed
    datareader_qos2.resource_limits().max_samples_per_instance = 13542;*/
   ...
   /*subscriber_qos.presentation().coherent_access = true;
    ...
    datareader_qos2.endpoint().unicast_locator_list.push_back(unicast_locator);
    ...
    datareader_qos2.endpoint().multicast_locator_list.push_back(multicast_locator);
   ...
   // topic_qos.durability().kind = VOLATILE_DURABILITY_QOS;
    ...
    {
        reader_ = subscriber_->create_datareader(topic_, datareader_qos2, &listener_);
    }

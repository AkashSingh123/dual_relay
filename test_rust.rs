...
fn main() {
  ...
  let mut writer_opt = if is_publisher {
    ...
    let mut writer = publisher
      .create_datawriter_cdr::<UInt8Type>(&topic, None) // None = get qos policy from publisher
      .unwrap();
    poll
      .register(
  ...
  let mut reader_opt = if is_subscriber {
    ...
    let mut reader = subscriber
      .create_datareader_cdr::<UInt8Type>(&topic, Some(qos))
      .unwrap();
    poll
  ...
  let mut data_sample = UInt8Type { value: 1 };
